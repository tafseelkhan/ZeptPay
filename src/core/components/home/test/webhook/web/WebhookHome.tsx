...
// Helper function to toggle event in selected events
const toggleEventInSelection = (
  selectedEvents: WebhookEvent[],
  eventName: string,
...
const fetchWebhook = async (token: string) => {
  try {
    console.log('📡 Fetching webhook...');
    const response = await axios.get(
      `http://172.20.10.12:7000/api/webhook/core/me`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
...
const fetchWebhookEvents = async () => {
  try {
    ...
    const response = await axios.get(
      `http://172.20.10.12:7000/api/webhook/core/events`,
      {
        timeout: 10000,
      },
    );
...
const createWebhook = async (
  ...
  try {
    ...
    const response = await axios.post(
      `http://172.20.10.12:7000/api/webhook/core/create`,
      { url, localUrl, events },
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
const updateWebhookEvents = async (token: string, events: WebhookEvent[]) => {
  try {
...
const toggleWebhookStatus = async (token: string, isActive: boolean) => {
  try {
    console.log('📡 Toggling webhook status...');
    const response = await axios.put(
      `http://172.20.10.12:7000/api/webhook/core/status`,
      { isActive },
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
const WebhookScreen: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setUserInfo(userData.data || userData.user);
      setEventCategories(eventsData.data || []);

      if (webhookData?.data) {
        setWebhook(webhookData.data);
        setSelectedEvents(webhookData.data.events || []);
      } else {
        setWebhook(null);
        setSelectedEvents([]);
      }
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      setWebhook(prev => (prev ? { ...prev, isActive: newStatus } : null));

      await toggleWebhookStatus(token, newStatus);
  ...
  const handleSaveEvents = async () => {
    ...
    try {
      ...
      const response = await updateWebhookEvents(token, selectedEvents);
  ...
  // Event Category Card Component with theme
  ...
  return (
    <SafeAreaProvider>
      ...
      >
        ...
        >
          {/* Header Section - Same as API Keys */}
          ...
          {!webhook ? (
            ...
            // Webhook Details - Card with Expand/Collapse
            <View style={styles.content}>
              {/* Webhook Card - Like API Key Card */}
              <View style={[styles.webhookCard, dynamicStyles.webhookCard]}>
                {/* Card Header - Always visible with down arrow */}
                <TouchableOpacity
                      ...
                      <View style={styles.sectionHeader}>
                        ...
                        <View style={styles.keyCountBadge}>
                          <Text style={styles.keyCountText}>
                            {getSelectedEventsCount()} selected
                          </Text>
                        </View>
                      </View>
...
// Styles - Static styles only (dynamic ones are handled in component)
